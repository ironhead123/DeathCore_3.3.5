...
void AnticheatMgr::Alerts(Player* player, uint8 alertType, uint8 hackType)
{
	...
	// AlertType 1
	std::string alertType1 = "|cFFFF0000You were caught by anticheat!\n|cFFFF0000...You will be punished if continue using hacking.";

	// AlertType 2
	std::string alertType2;
	alertType2 += ChatHandler(player->GetSession()).GetNameLink();
	alertType2 += "|cFFFF0000 was kicked by anticheat, reason: " + hType + " Hacking.";
	...
	case 1:
		...
		break;
	case 2:
		sWorld->SendServerMessage(SERVER_MSG_STRING, alertType2.c_str(), 0);
	default:
		return;
	}
}
...
void AnticheatMgr::Report(Player* player, uint8 hackType)
{
	...
	player->m_Events.AddEvent(new ClearTimes(player), player->m_Events.CalculateTime(5000));
...
{
	uint32 key = player->GetGUIDLow();
	bool enable = ConfigMgr::GetBoolDefault("Anticheat.Enable", false);
