...
public:
	...
	void OnLogin(Player *player, bool)
	{
		...
		if (rate != -1 && player->getLevel() != sWorld->getIntConfig(CONFIG_MAX_PLAYER_LEVEL))
		{
			...
			{
				if (uRate)
					ChatHandler(player->GetSession()).PSendSysMessage("|CFF7BBEF7[Individual Rates]|r: Your XP rate was set to %u.", uRate);
				else
					ChatHandler(player->GetSession()).SendSysMessage("|CFF7BBEF7[Individual Rates]|r: Your XP rate was set to 0. You won't gain any XP anymore.");
			}
		}
		...
		if (rate != -1)
		{
			...
			{
				if (uRate)
					ChatHandler(player->GetSession()).PSendSysMessage("|CFF7BBEF7[Individual Rates]|r: Your loot rate was set to %u.", uRate);
				else
					ChatHandler(player->GetSession()).SendSysMessage("|CFF7BBEF7[Individual Rates]|r: Your loot rate was set to 0. You won't be able to loot anything.");
			}
		}
	}
};
...
public:
	...
	static bool HandleRateXpCommand(ChatHandler *handler, const char *args)
	{
		...
		{
			handler->SendSysMessage("|CFF7BBEF7[Individual Rates]|r: You are already at maximum level.");
			return true;
		}
		...
		if (!*args)
		{
			handler->PSendSysMessage("|CFF7BBEF7[Individual Rates]|r: Your current XP rate is %u.", me->GetCustomXpRate());
			return true;
		}
		...
		{
			handler->PSendSysMessage("|CFF7BBEF7[Individual Rates]|r: Invalid rate specified, must be in interval [0,%i].", maxRate);
			handler->SetSentErrorMessage(true);
			return false;
		}
		...
		if (player == me)
			handler->PSendSysMessage("|CFF7BBEF7[Individual Rates]|r: You have set your XP rate to %u.", uRate);
		else
		{
			handler->PSendSysMessage("|CFF7BBEF7[Individual Rates]|r: You have set %s's XP rate to %u.", handler->GetNameLink(player).c_str(), uRate);
			ChatHandler(player->GetSession()).PSendSysMessage("|CFF7BBEF7[Individual Rates]|r: %s has set your XP rate to %u.", handler->GetNameLink().c_str(), uRate);
		}
	...
	static bool HandleRateLootCommand(ChatHandler *handler, const char *args)
	{
		...
		if (!*args)
		{
			handler->PSendSysMessage("|CFF7BBEF7[Individual Rates]|r: Your current loot rate is %u.", me->GetCustomLootRate());
			return true;
		}
		...
		// show a message indicating custom XP rate change
		if (player == me)
			handler->PSendSysMessage("|CFF7BBEF7[Individual Rates]|r: You have set your loot rate to %u.", uRate);
		else
		{
			handler->PSendSysMessage("|CFF7BBEF7[Individual Rates]|r: You have set %s's loot rate to %u.", handler->GetNameLink(player).c_str(), uRate);
			ChatHandler(player->GetSession()).PSendSysMessage("|CFF7BBEF7[Individual Rates]|r: %s has set your loot rate to %u.", handler->GetNameLink().c_str(), uRate);
		}
